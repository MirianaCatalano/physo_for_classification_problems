        ...
        def parser(i):
            ...
            # Arity = 1 ---
                ...
                # dimensionless (as it should be) just in case and that current token is dimensionless
                    ...
                    '''
                    print("--------")
                    print("\nDIMENSIONLESS ERROR DEBUG")
                    print("Token:", programs.lib_names[programs.tokens.idx[prog_i, i]])
                    print("Position:", i)

                    child_pos = programs.tokens.children_pos[prog_i, i][0]

                    print("Child token:", programs.lib_names[programs.tokens.idx[prog_i, child_pos]])
                    print("Child units:", child_phy_units)
                    print("Expected units:", np.zeros(Tok.UNITS_VECTOR_SIZE))
                    print("Is constraining:", child_is_constraining)
                    print("--------")
                    ...
                    '''
                    assert np.allclose(child_phy_units, np.zeros(Tok.UNITS_VECTOR_SIZE), atol=1e-15) and \
                        child_is_constraining == True, error_msg_dimensionless_child
                    assert np.allclose(phy_units, np.zeros(Tok.UNITS_VECTOR_SIZE), atol=1e-15) and \
                        is_constraining == True, error_msg_dimensionless_token
            # Arity = 2 ---
            elif arity == 2:
                # Children positions
                ...
                if Func.UNIT_BEHAVIORS_DICT["BINARY_ADDITIVE_OP"].is_id(behavior_id):
                    '''
                    print("--------")
                    print("child0 units:", child0_phy_units)
                    print("child1 units:", child1_phy_units)
                    print("difference:", child0_phy_units - child1_phy_units)
                    print("equal:", np.array_equal(child0_phy_units, child1_phy_units))
                    print("allclose:", np.allclose(child0_phy_units, child1_phy_units))
                    print("token:", programs.lib_names[programs.tokens.idx[prog_i, i]])
                    print("--------")
                    '''
                    #assert np.array_equal(child1_phy_units, child0_phy_units), error_msg_additive_discrepancy
                    assert np.allclose(child1_phy_units, child0_phy_units, atol=1e-15), error_msg_additive_discrepancy
